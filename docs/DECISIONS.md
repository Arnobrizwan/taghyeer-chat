...
- **A queued message gets the palette's amber "waiting" treatment, not the accent fill** — the same solid vermilion as a delivered one with a caption underneath — the product's single claim is that it does not pretend an undelivered message was delivered, and an identical bubble made the UI contradict the copy.
- **One pinned `Intl.DateTimeFormat` (`en-GB`, `hourCycle: 'h23'`) for every clock** — the runtime locale — literal seed timestamps rendered 24-hour beside live ones rendering 12-hour on the same screen.
- **The landing page shows the app in markup, not a screenshot** — a PNG — a still image is pinned to one theme on a page whose palette inverts, goes stale the moment the product moves, and costs orders of magnitude more bytes than reusing the classes it depicts.
- **`Reveal` reveals on a 3s failsafe and on an already-in-viewport check** — observer only — content that starts at `opacity: 0` and never receives its event is a blank page, which is a far worse failure than a missing animation.
